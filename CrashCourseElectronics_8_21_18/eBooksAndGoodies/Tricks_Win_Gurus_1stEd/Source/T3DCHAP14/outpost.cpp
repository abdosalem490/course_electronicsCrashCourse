...
for (int index=0; index < MAX_POWERUPS; index++)
    ...
    if (powerups[index].state == POWERUP_STATE_ALIVE)
        {
        // check for collision with player
        if (Collision_Test(player_x-(wraith.width>>1), 
                           player_y-(wraith.height>>1), 
                            wraith.width, wraith.height,
                            powerups[index].varsI[INDEX_WORLD_X]-(powerups[index].width>>1), 
			     		    powerups[index].varsI[INDEX_WORLD_Y]-(powerups[index].height>>1),
...
for (int mine=0; mine < MAX_MINES; mine++)
     ...
     if (mines[mine].state==MINE_STATE_ALIVE && 
         ...
         {
         // test for collision 
         if (Collision_Test(plasma[index].varsI[INDEX_WORLD_X]-(plasma[index].width>>1), 
                            plasma[index].varsI[INDEX_WORLD_Y]-(plasma[index].height>>1), 
                            plasma[index].width, plasma[index].height,
                            mines[mine].varsI[INDEX_WORLD_X]-(mines[mine].width>>1), 
			     		    mines[mine].varsI[INDEX_WORLD_Y]-(mines[mine].height>>1),
 ...
 for (int gunship=0; gunship < MAX_GUNSHIPS; gunship++)
     ...
     if (gunships[gunship].state==GUNSHIP_STATE_ALIVE && 
         ...
         if (Collision_Test(plasma[index].varsI[INDEX_WORLD_X]-(plasma[index].width>>1), 
                            plasma[index].varsI[INDEX_WORLD_Y]-(plasma[index].height>>1), 
                            plasma[index].width, plasma[index].height,
                            gunships[gunship].varsI[INDEX_WORLD_X]-(gunships[gunship].width>>1), 
			     		    gunships[gunship].varsI[INDEX_WORLD_Y]-(gunships[gunship].height>>1),
 ...
 for (int station=0; station < MAX_STATIONS; station++)
     ...
     if (stations[station].state == STATION_STATE_ALIVE && 
         ...
         if ( //stations[station].anim_state == STATION_SHIELDS_ANIM_OFF &&
             Collision_Test(plasma[index].varsI[INDEX_WORLD_X]-(plasma[index].width>>1), 
                            plasma[index].varsI[INDEX_WORLD_Y]-(plasma[index].height>>1), 
                            plasma[index].width, plasma[index].height,
                            stations[station].varsI[INDEX_WORLD_X]-(stations[station].width>>1), 
			     		    stations[station].varsI[INDEX_WORLD_Y]-(stations[station].height>>1),
        ...
        for (int rock=0; rock < MAX_ROCKS; rock++)
            ...
            if (rocks[rock].state==ROCK_STATE_ON)
                ...
                if (Collision_Test(plasma[index].varsI[INDEX_WORLD_X]-(plasma[index].width>>1), 
					               plasma[index].varsI[INDEX_WORLD_Y]-(plasma[index].height>>1), 
                                   plasma[index].width, plasma[index].height,
                                   rocks[rock].varsI[INDEX_WORLD_X]-(rocks[rock].width>>1), 
								   rocks[rock].varsI[INDEX_WORLD_Y]-(rocks[rock].height>>1),
                    ...
                    switch(rocks[rock].varsI[0])
                          ...
                          case ROCK_LARGE:
                              ...
                              Start_Burst(plasma[index].varsI[INDEX_WORLD_X], plasma[index].varsI[INDEX_WORLD_Y], 
                                          ...
                                          rocks[rock].xv>>1, rocks[rock].yv>>1);
...
for (int index=0; index < MAX_PLASMA; index++)
    ...
    if (plasma[index].state == PLASMA_STATE_OFF)
       ...
       plasma[index].varsI[INDEX_WORLD_X] = x-(plasma[0].width>>1);
       plasma[index].varsI[INDEX_WORLD_Y] = y-(plasma[0].height>>1);
...
for (int index=0; index < MAX_ROCKS; index++)
    ...
    if (rocks[index].state == ROCK_STATE_ON)
        ...
        // test for collision with rocks
        if (player_state == PLAYER_STATE_ALIVE && Collision_Test(player_x-(wraith.width>>1), player_y-(wraith.height>>1), 
                           wraith.width, wraith.height,
                           rocks[index].varsI[INDEX_WORLD_X]-(rocks[index].width>>1), 
						   rocks[index].varsI[INDEX_WORLD_Y]-(rocks[index].height>>1),
        ...
        switch(rocks[index].varsI[0])
              ...
              case ROCK_LARGE:
                   ...
                   Start_Burst(rocks[index].varsI[INDEX_WORLD_X], rocks[index].varsI[INDEX_WORLD_Y], 
                               68+rand()%12,54+rand()%10,
                               rocks[index].xv>>1, rocks[index].yv>>1);
               ...
               case ROCK_MEDIUM:
					...
					// update velocity vector
                    player_xv+=(rocks[index].xv>>1);
					player_yv+=(rocks[index].yv>>1);
                ...
                case ROCK_SMALL:
                     ...
                     Start_Burst(rocks[index].varsI[INDEX_WORLD_X], rocks[index].varsI[INDEX_WORLD_Y], 
                      ...
                      player_damage+=5;
  					  ...
  					  // update velocity vector
                      player_xv+=(rocks[index].xv>>2);
 					  player_yv+=(rocks[index].yv>>2);
...
for (int index=0; index<MAX_ROCKS; index++)
    ...
    if (rocks[index].state == ROCK_STATE_ON)
        {
        // transform to screen coords
        rocks[index].x = rocks[index].varsI[INDEX_WORLD_X] - (rocks[index].width>>1) - player_x + (SCREEN_WIDTH/2);
        rocks[index].y = rocks[index].varsI[INDEX_WORLD_Y] - (rocks[index].height>>1) - player_y + (SCREEN_HEIGHT/2);
...
for (int index=0; index < MAX_BURSTS; index++)
    ...
    if (bursts[index].state == BURST_STATE_ON)
        ...
        bursts[index].x = bursts[index].varsI[INDEX_WORLD_X] - (bursts[index].width>>1) - player_x + (SCREEN_WIDTH/2);
        bursts[index].y = bursts[index].varsI[INDEX_WORLD_Y] - (bursts[index].height>>1) - player_y + (SCREEN_HEIGHT/2);
